...
I used the [inquirer](https://github.com/SBoudrias/Inquirer.js) for that purpose, which itself aims for seemless user experience. It is used by several functions of the app to get user input such as the text input for the talks. The available options, based on the current state of the app, are generated by `src/main/getActionChoice.js` which passes the user choice in the form of a simple to keyword to `src/main/selectAction.js`. This function is the centerpiece of the application flow:
